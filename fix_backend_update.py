...
new_trigger = """    # Stage-aware action flows instead of a blind auto-flow hammer
    try:
        if status == "preparing":
            # Ensure tickets and WOs are created and submitted, but don't force material transfer/manufacture yet
            _run_sales_order_auto_flow(so_name, trigger="manual", force=True)
        elif status == "ready":
            # Only manufacture/complete WOs, if they exist
            ...
            # Only trigger Delivery Note creation if settings allow or force it
...
old_filter = """    filters = {"docstatus": 1}
...
new_filter = """    # DO NOT pre-filter on restaurant_status because canonical logic resolves it from documents.
...
api = api.replace(old_filter, new_filter)

with open('restaurant/api.py', 'w') as f:
    f.write(api)

print("Backend update fixed.")
