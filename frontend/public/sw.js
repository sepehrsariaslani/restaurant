const CACHE_VERSION = 'veederakht-pwa-v1'
  ...
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys
        .filter((key) => key.startsWith('veederakht-pwa-') && !key.startsWith(CACHE_VERSION))
...
function isStaticAsset(url) {
  return url.pathname.startsWith('/assets/restaurant/frontend/') ||
...
<body><main class="card"><h1>فعلاً آفلاین هستید</h1><p>اتصال اینترنت برقرار نیست. بعضی صفحات ذخیره‌شده ممکن است در دسترس باشند.</p><a href="/restaurant">تلاش دوباره</a></main></body>
</html>`, {
