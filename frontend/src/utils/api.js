...
const STATIC_DOCTYPE_COLUMNS = {
  Item: new Set(['name', 'item_code', 'item_name', 'item_group', 'standard_rate', 'disabled', 'custom_snapp_code']),
  'Item Group': new Set(['name', 'item_group_name', 'parent_item_group', 'is_group']),
}
...
async function listDoctypeColumns(doctype = '') {
  ...
  try {
    ...
    const set = new Set((rows || []).map((row) => String(row?.fieldname || '').trim()).filter(Boolean))
...
async function getMenuBootFallback() {
  ...
  try {
    rows = await callMethodByPath('frappe.client.get_list', {
      ...
      order_by: hasSortOrder ? 'restaurant_sort_order asc, item_group_name asc' : 'item_group_name asc',
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    const isSub = hasSubcategory
      ...
      : Number(row?.is_group || 0) !== 1 && Boolean(String(row?.parent_item_group || '').trim())
    const payload = {
      name: String(row?.name || '').trim(),
      title: String(row?.item_group_name || row?.name || '').trim(),
      slug: hasSlug ? String(row?.restaurant_slug || '').trim() : makeFallbackSlug(row?.item_group_name || row?.name || ''),
      image: '',
...
async function listManagementProductsFallback({ search = '', category = '', active_only = 0, branch = '', tag = '' } = {}) {
  ...
  const payload = (rows || []).map((row) => {
    ...
    const slug = String(row?.restaurant_slug || '').trim() || makeFallbackSlug(title) || makeFallbackSlug(itemCode)
    const basePrice = Number(row?.restaurant_base_price ?? row?.standard_rate ?? 0) || 0
    const isActive = itemHasRestaurantEnabled ? Number(row?.restaurant_enabled || 0) : Number(row?.disabled || 0) === 0 ? 1 : 0
...
export async function getManagementSessionProfile() {
  try {
    const user = String((await callMethodByPathGET('frappe.auth.get_logged_user')) || '').trim() || 'Guest'
    if (user === 'Guest') {
      return {
        user: 'Guest',
        full_name: '',
...
async function getMenuItemsFallback({
  ...
  if (normalizedSubcategory) {
    rows = rows.filter((row) => {
      ...
      const subcategoryNameSlug = makeFallbackSlug(row?.subcategory_title || row?.subcategory || '')
      return subcategorySlug === normalizedSubcategory || subcategoryNameSlug === normalizedSubcategory
    })
  }
...
export function getManagementBIReport(report_key, { date_from = '', date_to = '', compare_mode = 'previous_window' } = {}) {
...
export function listManagementPOSOrders({ date_from = '', date_to = '', status = '', cashier = '' } = {}) {
  return callRestaurantAPI('list_management_pos_orders', { date_from, date_to, status, cashier })
}

export function listManagementPOSPaymentLogs({ date_from = '', date_to = '', status = '', limit = 50 } = {}) {
  return callRestaurantAPI('list_management_pos_payment_logs', { date_from, date_to, status, limit })
}
...
export function placeTableOrder({
  qr_token = '',
  items = [],
  note = '',
} = {}) {
...
export function listManagementOrders({
  ...
  return callRestaurantAPI('list_management_orders', { date_from, date_to, status, source, cashier })
}
...
export async function uploadManagementItemImage({ item_name = '', file = null, is_private = 0 } = {}) {
  ...
  if (!response.ok || payload.exc || payload.exception) {
    const serverMessage = unpackServerMessages(payload)
    const message = serverMessage || payload._error_message || payload.message || 'آپلود تصویر ناموفق بود.'
    throw new Error(message)
  }
...
export async function listManagementModifierGroups({ search = '', limit = 300 } = {}) {
  ...
  const args = {
    ...
    fields: ['name', 'title', 'selection_mode', 'required', 'min_select', 'max_select', 'is_active', 'sort_order'],
    filters: [['is_active', '=', 1]],
...
function normalizeBomItems(items = []) {
  ...
  return (Array.isArray(items) ? items : [])
    .map((row) => ({
      ...
      restaurant_is_included_by_default: normalizeCheck(row?.restaurant_is_included_by_default, 1),
...
export async function createManagementBom(payload = {}) {
  ...
  const bomDoc = {
    ...
    conversion_rate: 1,
    with_operations: 0,
    rm_cost_as_per: String(payload?.rm_cost_as_per || 'Valuation Rate').trim() || 'Valuation Rate',
    ...
    items,
    restaurant_modifier_rows: normalizeBomModifierRows(payload?.restaurant_modifier_rows || []),
  }
...
export async function updateManagementBom(payload = {}) {
  ...
  const saved = await callMethodByPath('restaurant.api.save_management_bom', {
    payload: {
      ...
      rm_cost_as_per: String(payload?.rm_cost_as_per || current?.rm_cost_as_per || 'Valuation Rate').trim() || 'Valuation Rate',
      ...
      is_active: payload?.is_active === false ? 0 : 1,
      is_default: payload?.is_default ? 1 : 0,
      restaurant_nutrition_kcal: Number(payload?.restaurant_nutrition_kcal || current?.restaurant_nutrition_kcal || 0),
      restaurant_nutrition_protein_g: Number(payload?.restaurant_nutrition_protein_g || current?.restaurant_nutrition_protein_g || 0),
      restaurant_nutrition_carb_g: Number(payload?.restaurant_nutrition_carb_g || current?.restaurant_nutrition_carb_g || 0),
      restaurant_nutrition_sugar_g: Number(payload?.restaurant_nutrition_sugar_g || current?.restaurant_nutrition_sugar_g || 0),
      restaurant_nutrition_fat_g: Number(payload?.restaurant_nutrition_fat_g || current?.restaurant_nutrition_fat_g || 0),
...
export async function deleteManagementProduct(item_name, { allow_archive_on_link = 1, force_delete = 0 } = {}) {
  const itemName = String(item_name || '').trim()
  if (!itemName) {
    throw new Error('شناسه محصول معتبر نیست.')
  }
  ...
  try {
    ...
    const isMissingMethod =
      ...
      message.includes('Failed to get method for command restaurant.api.delete_management_product')
    ...
    try {
      ...
      const canArchive =
        ...
        /(linked|link exists|cannot delete|وابسته|مرتبط|reference|dependent)/i.test(deleteMessage)
      if (!canArchive) {
        throw deleteError
      }
...
function normalizeManagementMenuGroupPayload(payload = {}, currentDoc = null) {
  ...
  const normalized = {
    ...
    parent_item_group: String(source?.parent_item_group ?? fromDoc?.parent_item_group ?? '').trim(),
    is_group,
    ...
    restaurant_sort_order: Number(source?.restaurant_sort_order ?? fromDoc?.restaurant_sort_order ?? 0) || 0,
    restaurant_description: String(source?.restaurant_description ?? fromDoc?.restaurant_description ?? '').trim(),
...
function buildLegacyCustomerReport(orders = [], { date_from = '', date_to = '' } = {}) {
  ...
  const paidOrdersCount = orders.filter(
    (row) => String(row?.payment_status || '').trim().toLowerCase() === 'paid',
  ).length
  ...
  for (const order of orders) {
    ...
    for (const item of Array.isArray(order?.items) ? order.items : []) {
      ...
      const bucket = productGrouped.get(title) || { product_title: title, qty: 0, amount: 0 }
  ...
  return {
    ...
    kpis: [
      { key: 'customer_sales', label: 'کل خرید', value: totalSpent, unit: 'money', trend: 'flat', change_pct: 0 },
      { key: 'customer_orders', label: 'تعداد سفارش', value: ordersCount, unit: 'count', trend: 'flat', change_pct: 0 },
      { key: 'customer_avg_ticket', label: 'میانگین فاکتور', value: avgTicket, unit: 'money', trend: 'flat', change_pct: 0 },
      { key: 'customer_paid_rate', label: 'نرخ سفارش پرداخت شده', value: Number(paidRate.toFixed(2)), unit: 'percent', trend: 'flat', change_pct: 0 },
    ],
    charts: [],
    tables: [
      {
    ...
    insights: [
      { type: 'info', text: `سفارش پرداخت شده: ${paidOrdersCount} | پرداخت نشده: ${unpaidOrdersCount}` },
      { type: 'info', text: `بازه گزارش: ${date_from || '-'} تا ${date_to || '-'}` },
    ],
  }
}

async function getManagementCustomerDetailFallback({ mobile = '', customer_name = '', date_from = '', date_to = '' } = {}) {
  const normalizedMobile = String(mobile || '').trim()
  ...
  const resolvedName = normalizedCustomerName || normalizeCustomerLabel(lastOrder || firstOrder || {})
  const resolvedMobile = normalizedMobile || normalizeCustomerMobile(lastOrder || firstOrder || {})
  ...
  const unpaidOrdersCount = Math.max(ordersCount - paidOrdersCount, 0)
  const daysSinceLastOrder = lastOrder ? Math.max(0, Math.floor((Date.now() - getOrderTimestamp(lastOrder)) / 86400000)) : 0
...
export async function getManagementCustomerDetail({ mobile = '', customer_name = '', date_from = '', date_to = '' } = {}) {
