<template>
  <section class="pos-theme pos-fullpage" :class="{ 'focus-mode': focusMode }">
    ...
    <div class="ticket-tabs-bar">
      ...
      <button
        ...
        :title="focusMode ? 'خروج از حالت تمرکز' : 'حالت تمرکز صندوقدار'"
        @click="toggleFocusMode"
      >
        <span v-if="focusMode">◱ خروج از تمرکز</span>
        <span v-else>⛶ حالت تمرکز</span>
      </button>
    </div>

    <!-- Floating quick access buttons (focus mode only) -->
    <div v-if="focusMode" class="focus-quick-bar">
      <button type="button" class="focus-quick-btn" @click="openSidebarModal('tables')">
        🪑 میزها
        <span class="focus-quick-badge" v-if="occupiedTableCount">{{ occupiedTableCount }}</span>
      </button>
      <button type="button" class="focus-quick-btn" @click="openSidebarModal('invoices')">
        📋 فاکتورهای باز
        <span class="focus-quick-badge" v-if="openInvoices.length">{{ openInvoices.length }}</span>
      </button>
      <button type="button" class="focus-quick-btn" @click="openSidebarModal('recent')">
        🕐 سفارش‌های اخیر
      </button>
      <button type="button" class="focus-quick-btn" @click="openSidebarModal('history')">
        💰 تراکنش‌های امروز
      </button>
    </div>

    <section class="pos-shell" dir="rtl">
      <aside class="left-col" v-show="!focusMode">
    ...
    <!-- Sidebar Modal (Focus Mode) -->
    <div v-if="showSidebarModal" class="sidebar-modal-backdrop" @click.self="closeSidebarModal">
      <section class="sidebar-modal">
        <header class="sidebar-modal-header">
          <h3>{{ leftPanelTab === 'tables' ? 'میزها' : leftPanelTab === 'invoices' ? 'فاکتورهای باز' : leftPanelTab === 'history' ? 'تراکنش‌های امروز' : 'سفارش‌های اخیر' }}</h3>
          <button type="button" class="sidebar-modal-close" @click="closeSidebarModal">×</button>
        </header>

        <div class="sidebar-modal-tabs">
          <button
            type="button"
            :class="{ active: leftPanelTab === 'tables' }"
            @click="leftPanelTab = 'tables'"
        ...
        <div class="sidebar-modal-body">
          <!-- Tables Tab -->
          <section v-if="leftPanelTab === 'tables'" class="modal-table-section">
            <div class="tab-panel-toolbar">
              <button type="button" class="icon-refresh-btn" @click="loadPOSBoot" title="بروزرسانی">↻</button>
            </div>
            <div class="table-grid">
              <article
                ...
                class="table-cell"
                :class="[`status-${String(table.status || '').toLowerCase()}`, { active: selectedDineInTable?.name === table.name }]"
                @click="selectDineInTable(table); closeSidebarModal()"
              >
                <span class="table-cell-name">{{ table.label }}</span>
                <span class="table-cell-time" v-if="table.occupied_minutes">{{ formatOccupiedMinutes(table.occupied_minutes) }}</span>
                <span class="table-cell-orders" v-if="table.pending_orders">{{ toFaDigits(table.pending_orders) }} سفارش</span>
              ...
              <div class="table-detail-bar">
                ...
                <div class="table-detail-actions">
                  ...
                  <template v-if="selectedTablePreview?.session?.name">
                    ...
                    <div class="tbl-action-row" v-if="mergeableTableDropdownOptions.length">
                      <SearchableDropdown v-model="mergeTableTarget" :options="mergeableTableDropdownOptions" placeholder="ادغام با..." search-placeholder="جستجو..." include-empty-option empty-label="ادغام با..." tone="dark" />
                      <button type="button" class="tbl-btn primary" :disabled="!mergeTableTarget" @click="mergeSelectedTableSession">ادغام</button>
                    </div>
                  </template>
                </div>
              </div>
          ...
          <!-- Open Invoices Tab -->
          <section v-if="leftPanelTab === 'invoices'" class="modal-invoice-section">
            <p class="muted" v-if="!openInvoices.length">فاکتور بازی نیست.</p>
            <div class="history-list" v-else>
              <button
                ...
                @click="openOrderDetailModal(inv); closeSidebarModal()"
              >
                <div class="history-card-head">
                  <strong>{{ inv.order_code || inv.name }}</strong>
                  <span class="history-time">{{ formatInvoiceDateTime(inv.created_at) }}</span>
                </div>
                <div class="history-card-body">
                  <span class="history-amount">{{ formatMoney(inv.grand_total || 0, currency) }}</span>
                  <span class="history-method-badge">{{ inv.payment_method || 'نامشخص' }}</span>
                </div>
              </button>
            </div>
          </section>
          ...
          <section v-if="leftPanelTab === 'history'" class="modal-history-section">
            <p class="muted" v-if="todayTransactionsLoading">در حال دریافت...</p>
            <p class="error" v-else-if="todayTransactionsError">{{ todayTransactionsError }}</p>
            <p class="muted" v-else-if="!todayTransactions.length">تراکنشی نیست.</p>
            <div class="history-list" v-else>
              <button
                ...
                type="button"
                class="history-card clickable"
                @click="openOrderDetailModal(tx); closeSidebarModal()"
              >
                <div class="history-card-head">
                  <strong>{{ tx.order_code || tx.name }}</strong>
                  <span class="history-time">{{ formatInvoiceDateTime(tx.created_at) }}</span>
                ...
                <div class="history-card-body">
                  <span class="history-amount">{{ formatMoney(tx.grand_total || 0, currency) }}</span>
                  <span class="history-method-badge">{{ tx.payment_method || 'نامشخص' }}</span>
                </div>
              </button>
            </div>
          </section>
          ...
          <!-- Recent Orders Tab -->
          <section v-if="leftPanelTab === 'recent'" class="modal-recent-section">
            <div class="recent-date-filter">
              <input
                type="date"
                class="input dark-input"
                :value="recentOrdersDateFrom"
                @change="recentOrdersDateFrom = $event.target.value; loadRecentOrders()"
              />
            </div>
            <p class="muted" v-if="recentOrdersLoading">در حال دریافت...</p>
            <p class="error" v-else-if="recentOrdersError">{{ recentOrdersError }}</p>
            <p class="muted" v-else-if="!recentOrders.length">سفارشی نیست.</p>
            <div class="history-list" v-else>
              <button
                ...
                type="button"
                class="history-card clickable"
                @click="openOrderDetailModal(ord); closeSidebarModal()"
              >
                <div class="history-card-head">
                  <strong>{{ ord.order_code || ord.name }}</strong>
                  <span class="history-time">{{ formatInvoiceDateTime(ord.created_at) }}</span>
                </div>
                <div class="history-card-body">
                  <span class="history-amount">{{ formatMoney(ord.grand_total || 0, currency) }}</span>
                  <span class="history-method-badge">{{ ord.payment_method || 'نامشخص' }}</span>
                </div>
              </button>
            </div>
          </section>
...
const focusMode = ref(loadFocusModePreference())
const showSidebarModal = ref(false)

function loadFocusModePreference() {
  try {
    return localStorage.getItem('pos-focus-mode') === '1'
  } catch {
    return false
  }
}
...
function toggleFocusMode() {
  ...
  try {
    localStorage.setItem('pos-focus-mode', focusMode.value ? '1' : '0')
  } catch {
    // ignore
  }
  if (!focusMode.value) {
...
function openSidebarModal(tab) {
  if (tab) {
    leftPanelTab.value = tab
    if (tab === 'history') loadTodayTransactions()
    if (tab === 'recent') loadRecentOrders()
  }
...
.pos-theme.focus-mode .pos-shell {
  grid-template-columns: minmax(0, 1fr) 460px;
  padding: 0 0.5rem;
}

.pos-theme.focus-mode .left-col {
...
.sidebar-modal-tabs button.active {
  color: var(--pos-primary);
  border-bottom-color: var(--pos-primary);
  background: rgb(var(--pos-primary-rgb, 1 90 114) / 0.05);
}

.sidebar-modal-tabs button:hover:not(.active) {
  background: rgb(var(--pos-primary-rgb, 1 90 114) / 0.04);
}
...
.modal-table-section,
.modal-invoice-section,
.modal-history-section,
.modal-recent-section {
  display: grid;
  gap: 0.6rem;
  align-content: start;
}
