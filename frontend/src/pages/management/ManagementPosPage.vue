    ...
    <div v-if="orderDetailModal.open" class="pos-modal-backdrop" @click.self="closeOrderDetailModal">
      ...
      <section class="pos-modal" dir="rtl">
        ...
        <template v-else-if="orderDetailModal.order">
          ...
          <!-- Settlement Section for unpaid orders -->

          <div class="order-settle-section" v-if="orderDetailModal.canSettle">

            <h4>{% raw %}تسویه سفارش{% endraw %}</h4>
...
function canSettleOrder(order) {
  ...
  const unpaidStatuses = ['pending', 'confirmed', 'preparing', 'ready', 'served', 'courier_handoff', 'on_the_way']

  return unpaidStatuses.includes(status) && !order.payment_method
...
async function submitPOSOrder(payNow = true, paymentMeta = {}) {
  ...
  try {
    ...
    // Refresh open invoices and recent orders
    loadOpenInvoices()
    loadRecentOrders()
...
async function quickSettleOrder(order) {
  ...
  // Open the detail modal with settle section active
