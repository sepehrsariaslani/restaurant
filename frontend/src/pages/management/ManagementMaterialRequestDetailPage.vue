<template>
  <InventorySectionShell :title="isNew ? 'درخواست مواد جدید' : request?.name || 'جزئیات درخواست مواد'" subtitle="ثبت نیاز مواد، بررسی وضعیت و انتقال به خرید">
    ...
    <ManagementSurfaceCard v-if="isNew || editing" title="فرم درخواست مواد" subtitle="واحد ماده بعد از انتخاب، خودکار از ERPNext خوانده می‌شود.">
      ...
      <div class="request-lines"><header><strong>اقلام درخواست</strong><button type="button" class="secondary-btn" @click="addLine">+ افزودن قلم</button></header><div v-for="(line,index) in form.items" :key="index" class="request-line"><SearchableDropdown v-model="line.item_code" allow-item-create :options="materialOptions" placeholder="انتخاب ماده..." search-placeholder="جستجوی ماده..." @update:model-value="syncLine(line,$event)" @item-created="applyCreatedItem(line,$event)" /><input class="input" type="number" min="0.001" step="0.001" v-model.number="line.qty" placeholder="مقدار" /><SearchableDropdown v-model="line.uom" :options="uomOptions" placeholder="واحد" search-placeholder="جستجوی واحد..." @update:model-value="line.conversion_factor = 0" /><button type="button" class="tertiary-btn danger" @click="removeLine(index)" :disabled="form.items.length===1">حذف</button></div></div>
      <p v-if="!hasValidItems" class="form-hint">برای ذخیره، حداقل یک ماده را از فهرست انتخاب کنید و مقدار آن را وارد کنید.</p><footer class="actions"><button type="button" class="secondary-btn" @click="save(false)" :disabled="saving || !hasValidItems">ذخیره پیش‌نویس</button><button type="button" class="primary-btn" @click="save(true)" :disabled="saving || !hasValidItems">{{ saving ? 'در حال ثبت...' : 'ثبت نهایی درخواست' }}</button></footer>
...
import { createManagementPurchaseFromMaterialRequest, getManagementInventoryBoot, getManagementMaterialRequest, getManagementMaterialRequestPrint, listManagementRawMaterials, listManagementUOMs, saveManagementMaterialRequest, updateManagementMaterialRequestStatus } from '@/utils/api'
...
function syncLine(line, code=line.item_code) { line.item_code=code; const meta=(boot.value?.materials||[]).find((item)=>item.name===code); if(meta){line.uom=meta.stock_uom||'Nos';line.stock_uom=meta.stock_uom||line.uom;line.conversion_factor=1;if(line.uom&&!uoms.value.includes(line.uom))uoms.value=[line.uom,...uoms.value]} }
function applyCreatedItem(line, item) { line.item_code=item.item_code||item.name; line.uom=item.stock_uom||line.uom||'Nos'; line.stock_uom=line.uom; line.conversion_factor=1; if (boot.value?.materials && !boot.value.materials.some((row)=>row.name===line.item_code)) boot.value.materials.unshift({name:line.item_code,item_name:item.item_name,stock_uom:line.uom}) }
function applyRequest(value, linked=[]) { request.value=value; purchases.value=linked; editing.value=false; const items=(value.items||[]).map((line)=>({...line,conversion_factor:Number(line.conversion_factor||1)})); items.forEach((line)=>{if(line.uom&&!uoms.value.includes(line.uom))uoms.value.push(line.uom)}); Object.assign(form,{name:value.name,transaction_date:value.transaction_date||today(),schedule_date:value.schedule_date||value.transaction_date||today(),set_warehouse:value.set_warehouse||'',note:value.note||'',items}) }
async function load(){loading.value=true;error.value='';try{const [bootPayload, materialsPayload, uomPayload]=await Promise.all([getManagementInventoryBoot(),listManagementRawMaterials({limit:500, include_all_stock: 1}),listManagementUOMs({limit:500})]);boot.value={...bootPayload,materials:materialsPayload.items||[]};uoms.value=uomPayload.uoms||[];if(!isNew){const payload=await getManagementMaterialRequest(requestName);applyRequest(payload.request,payload.purchase_orders||[])}}catch(err){error.value=err.message||'دریافت درخواست ناموفق بود.'}finally{loading.value=false}}
...
<style scoped>
.form-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:.65rem}.form-grid label{display:grid;gap:.25rem;color:var(--mg-text-main);font-size:.78rem}.full{grid-column:1/-1}.request-lines{display:grid;gap:.55rem;margin-top:1rem;padding:.8rem;border:1px dashed var(--mg-border);border-radius:15px}.request-lines header{display:flex;align-items:center;justify-content:space-between;gap:.5rem}.request-line{display:grid;grid-template-columns:minmax(0,3fr) minmax(90px,1fr) 90px auto;gap:.45rem;align-items:center}.uom{min-height:38px;display:grid;align-content:center;gap:.1rem;padding:.25rem .45rem;border:1px solid var(--mg-border-light);border-radius:9px}.uom small,.detail-line small,.detail-meta small{color:var(--mg-text-muted);font-size:.67rem}.form-hint{margin:.65rem 0 0;color:var(--mg-text-muted);font-size:.74rem}.actions{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:.5rem;margin-top:1rem;padding-top:.7rem;border-top:1px solid var(--mg-border-light)}.detail-meta{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:.55rem}.detail-meta>div{display:grid;gap:.18rem;padding:.6rem;border-radius:12px;background:var(--mg-bg-page)}.detail-meta strong{font-size:.8rem}.success{color:var(--mg-success)}.danger,.error{color:var(--mg-danger)}.primary{color:var(--mg-primary)}.detail-lines{display:grid;gap:.45rem;margin-top:1rem}.detail-line{display:flex;justify-content:space-between;align-items:center;gap:.6rem;padding:.65rem .7rem;border:1px solid var(--mg-border-light);border-radius:12px}.detail-line>div{min-width:0;display:grid;gap:.1rem}.detail-line strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.detail-line small{display:block}.line-qty{color:var(--mg-success);white-space:nowrap}.note,.linked-purchases{margin-top:.8rem;padding:.65rem;border-radius:12px;background:var(--mg-bg-page);white-space:pre-line}.linked-purchases{display:grid;gap:.35rem}.linked-purchases button{width:fit-content;border:0;background:transparent;color:var(--mg-primary);font:inherit;cursor:pointer}@media(max-width:650px){.request-line{grid-template-columns:1fr 1fr}.request-line .uom,.request-line .tertiary-btn{min-height:38px}.request-line :deep(.searchable-dropdown){grid-column:1/-1}.actions>*{flex:1 1 100%}.detail-meta{grid-template-columns:1fr 1fr}}@media(max-width:390px){.detail-meta{grid-template-columns:1fr}}
</style>
