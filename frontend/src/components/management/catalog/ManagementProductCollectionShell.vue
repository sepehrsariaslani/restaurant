<template>
  <section class="product-reference-shell product-reference-shell--collection">
    <header v-if="showHeader" class="product-reference-shell__header">
      <div>
        <span class="product-reference-shell__eyebrow">قالب مرجع مجموعه</span>
        <h2>{{ title }}</h2>
    ...
    <div v-if="$slots.toolbar" class="product-reference-shell__toolbar">
      <slot name="toolbar" />
    </div>
...
defineProps({
  ...
  loadingLabel: {
    type: String,
    default: 'در حال بارگذاری مجموعه...',
  },
  retryLabel: {
    type: String,
    default: 'تلاش دوباره',
...
.product-reference-shell__toolbar,
