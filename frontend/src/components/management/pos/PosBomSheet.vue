    ...
    <section class="sheet-card">
      ...
      <template v-else-if="item">
        <div class="qty-row">
          <small>تعداد</small>
          <div class="counter">
            <button type="button" @click="$emit('update:qty', Math.max(Number(qty || 1) - 1, 1))">-</button>
            <strong>{{ qty }}</strong>
            <button type="button" @click="$emit('update:qty', Number(qty || 1) + 1)">+</button>
          </div>
        </div>
        ...
        <footer class="sheet-foot">
          ...
          <button type="button" class="submit-btn" @click="$emit('confirm')">{{ confirmLabel }}</button>
        </footer>
      </template>
    </section>
  </div>
