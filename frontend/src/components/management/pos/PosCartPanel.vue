<template>
  <section class="cart-panel">
    ...
    <section v-if="orderMode === 'dine_in'" class="table-orders-box">
      <header class="table-orders-head">
        <h3>سفارش‌های میز {{ selectedTableLabel || '-' }}</h3>
        <button
          type="button"
          class="secondary-btn"
          :disabled="!confirmedTableOrders.length"
          @click="$emit('print-confirmed-table')"
        >
          چاپ تاییدشده‌ها
        </button>
      </header>

      <p class="hint" v-if="tablePreviewLoading">در حال دریافت سفارش‌های میز...</p>
      <div class="table-order-columns" v-else>
        <article class="table-order-column">
          <header>
            <strong>سفارش‌های عادی</strong>
            <small>{{ pendingTableOrders.length }}</small>
          </header>
          <p v-if="!pendingTableOrders.length" class="hint">سفارش عادی ندارد.</p>
          <div v-else class="table-order-list">
            <section v-for="order in pendingTableOrders" :key="order.name" class="table-order-card">
              <div class="table-order-card-head">
                <strong>{{ order.order_code }}</strong>
                <small>{{ formatDateTime(order.created_at) }}</small>
              </div>
              <ul>
                <li v-for="item in order.items || []" :key="item.row_name || `${order.name}-${item.menu_item}`">
                  <span>{{ item.menu_item_title }}</span>
                  <div class="item-counter">
                    <button
                      type="button"
                      @click="
                        $emit('update-table-order-item', {
                          order,
                          item,
                          delta: -1,
                        })
                    ...
                    <span>{{ formatCompactNumber(item.quantity, 2) }}</span>
                    <button
                      type="button"
                      @click="
                        $emit('update-table-order-item', {
        ...
        <article class="table-order-column confirmed">
          <header>
            <strong>سفارش‌های تاییدشده</strong>
            <small>{{ confirmedTableOrders.length }}</small>
          </header>
          <p v-if="!confirmedTableOrders.length" class="hint">سفارش تاییدشده ندارد.</p>
          <div v-else class="table-order-list">
            <section v-for="order in confirmedTableOrders" :key="`confirmed-${order.name}`" class="table-order-card">
              <div class="table-order-card-head">
                <strong>{{ order.order_code }}</strong>
    ...
    <section class="financial-box">
      ...
      <div class="field inline" v-if="paymentMethod === 'card'">
        <span>مرجع و RRN</span>
        <div class="code-row two">
          <input
            class="input dark-input"
            :value="paymentReference"
            @input="$emit('update:paymentReference', $event.target.value)"
            placeholder="مرجع"
          />
          <input
            class="input dark-input"
            :value="paymentRrn"
            @input="$emit('update:paymentRrn', $event.target.value)"
            placeholder="RRN"
          />
        </div>
      </div>
