<template>
  <div class="ingredient-section" :class="{ 'is-preview': isPreview }">
    <!-- Section Header -->
    ...
    <div class="ingredient-grid" v-if="filteredIngredients.length">
      ...
      >
        ...
        <div class="card-content">
          ...
          <div class="card-info">
            <div class="card-name">{{ ingredient.customer_label || ingredient.name }}</div>
            <div class="card-qty">{{ ingredient.base_qty }} {{ ingredient.qty_uom || '' }}</div>
            <div class="card-meta" v-if="hasIngredientMeta(ingredient)">
              <span class="meta-kcal" v-if="hasPositiveKcal(ingredient)">
                <Flame class="meta-kcal-icon" :size="13" /> {{ formatKcal(ingredient.nutrition_kcal) }}
              </span>
              <span class="meta-extra" v-if="Number(ingredient.extra_when_added || 0)">
          ...
          <div class="card-img" @click="onImageClick(ingredient)">
            <img
        ...
        <div class="card-actions" v-if="!isPreview && (canEditQty(ingredient) || canQuickRemove(ingredient))">
          <template v-if="canEditQty(ingredient)">
            <button class="qty-btn" type="button" :disabled="isDecDisabled(ingredient)" @click="decrease(ingredient)" aria-label="کاهش">−</button>
            <span class="qty-val">x{{ getMultiplier(ingredient) }}</span>
            <button class="qty-btn" type="button" :disabled="isIncDisabled(ingredient)" @click="increase(ingredient)" aria-label="افزایش">+</button>
          </template>
          <button
    ...
    <div class="nutrition-summary" v-if="!isPreview && filteredIngredients.length && hasAnyNutrition">
      ...
      <div class="nutrition-grid">
        ...
        <div class="nutrition-item" v-if="totalNutrition.protein_g > 0">
          ...
          <strong class="nutrition-value">{{ Math.round(totalNutrition.protein_g) }}</strong>
        ...
        <div class="nutrition-item" v-if="totalNutrition.carb_g > 0">
          ...
          <strong class="nutrition-value">{{ Math.round(totalNutrition.carb_g) }}</strong>
          <span class="nutrition-unit">g</span>
        </div>
    ...
    <!-- Empty states -->
    <p class="empty-msg" v-else-if="!isPreview && ingredients.length">ماده‌ای با این جستجو پیدا نشد.</p>
    <p class="empty-msg" v-else-if="!isPreview">برای این محصول ماده قابل تنظیم تعریف نشده است.</p>
  </div>
</template>
...
.card-img {
  ...
  overflow: hidden;
  cursor: pointer;
  background: linear-gradient(135deg, rgb(var(--palette-deep-saffron-rgb, 201,141,66) / 0.05), rgb(var(--palette-deep-sapphire-rgb, 111,74,49) / 0.03));
}
...
.card-img-placeholder {
  ...
  background: linear-gradient(135deg, rgb(var(--palette-deep-saffron-rgb, 201,141,66) / 0.1), rgb(var(--palette-deep-sapphire-rgb, 111,74,49) / 0.05));
...
.is-preview .card-img-placeholder {
  ...
  align-items: center;
  justify-content: center;
  background: linear-gradient(135deg, rgb(var(--palette-deep-sapphire-rgb, 111,74,49) / 0.05), rgb(var(--palette-deep-saffron-rgb, 201,141,66) / 0.04));
  color: var(--accent-green);
  font-weight: 700;
  font-size: 1.1rem;
  border-radius: 10px;
