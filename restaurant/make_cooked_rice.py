...
def make_cooked_rice():
    ...
    if not frappe.db.exists('Item', item_name):
        ...
        frappe.db.set_value('Item', item_name, {'restaurant_nutrition_kcal': 1.30, 'restaurant_nutrition_protein_g': 0.027, 'restaurant_nutrition_fat_g': 0.003, 'restaurant_nutrition_carb_g': 0.28})
        frappe.db.commit()
    ...
    bom.append('items', {'item_code': 'برنج سفید', 'qty': 100, 'uom': 'گرم'})
    bom.insert()
    frappe.db.commit()
    ...
    for bom_info in raw_rice_boms:
        ...
        for itm in list(bom_doc.items):
            if itm.item_code == 'برنج سفید':
                ...
                break
        bom_doc.append('items', {'item_code': item_name, 'qty': raw_qty, 'uom': 'گرم'})
        bom_doc.save()
        frappe.db.commit()
