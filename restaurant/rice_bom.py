...
def make_cooked_rice():
    ...
    # Create BOM: 100g raw rice -> 250g cooked rice
    ...
    # Replace raw rice with cooked rice in all BOMs
    raw_rice_boms = frappe.db.sql(
        'SELECT DISTINCT b.name, b.item, bi.qty as raw_qty '
        'FROM `tabBOM` b JOIN `tabBOM Item` bi ON bi.parent = b.name '
        'WHERE bi.item_code = "برنج سفید" AND b.is_active = 1',
        as_dict=True
    )
    ...
    for bom_info in raw_rice_boms:
        ...
        # Add cooked rice with same weight
