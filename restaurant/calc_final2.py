...
def calc_and_update():
    ...
    for bp in bom_products:
        ...
        for it in items:
            ...
            conv = get_uom_conversion(it.item_code, it.uom)
            adj_qty = qty * conv
            total_kcal += float(it.restaurant_nutrition_kcal or 0) * adj_qty
            total_p += float(it.restaurant_nutrition_protein_g or 0) * adj_qty
            total_f += float(it.restaurant_nutrition_fat_g or 0) * adj_qty
            total_c += float(it.restaurant_nutrition_carb_g or 0) * adj_qty
        bom_qty = float(bom.quantity or 1)
        kcal = round(total_kcal / bom_qty, 1)
        protein = round(total_p / bom_qty, 1)
        fat = round(total_f / bom_qty, 1)
        carbs = round(total_c / bom_qty, 1)
        try:
